...
it('lets user2 buy a star and decreases its balance in ether', async() => {
  ...
  it('Check name and symbole', async() => {
    let instance = await StarNotary.deployed();
    assert.equal(await instance.name.call(), 'StarTrack');
    assert.equal(await instance.symbol.call(), 'STRT');
});
...
it('lets user1 and user2 exchange their stars', async() => {
  ...
  it('lets transfer star to user2', async() => {
    let instance = await StarNotary.deployed();
    let user1 = accounts[1];
    let user2 = accounts[2];
    let tokenId = 101;
